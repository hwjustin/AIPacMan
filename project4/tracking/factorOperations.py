...
def joinFactors(factors: ValuesView[Factor]):
    ...
    result_factor = Factor(result_uncond, result_cond, result_dict)
    
    for assignmentDict in result_factor.getAllPossibleAssignmentDicts():
        probability = 1
        for factor in factors:
            probability *= factor.getProbability(assignmentDict)
        result_factor.setProbability(assignmentDict, probability)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        "*** YOUR CODE HERE ***"
        # The variables needed for result factors
        result_uncond = factor.unconditionedVariables()
        result_uncond.remove(eliminationVariable)
        result_cond = factor.conditionedVariables()
        result_dict = factor.variableDomainsDict()

        result_factor = Factor(result_uncond, result_cond, result_dict)
        for assignmentDict in factor.getAllPossibleAssignmentDicts():
            original = result_factor.getProbability(assignmentDict)
            original += factor.getProbability(assignmentDict)
            result_factor.setProbability(assignmentDict, original)

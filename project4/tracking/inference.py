...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            pac_range.append((i, j))
    


    variableDomainsDict[PAC]  = pac_range
    variableDomainsDict[GHOST0]  = pac_range
    variableDomainsDict[GHOST1]  = pac_range
    variableDomainsDict[OBS0]  = [i for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)]
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        for key in self.keys():
            self[key] = self[key] / float(weight)
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Not Sure, do we need to normalize it?
        total = self.total()
        random_value = random.random()
        random_value *= total
        for key in self.keys():
            random_value -= self[key]
            if random_value < 0:
                return key
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        t_dis = manhattanDistance(pacmanPosition, ghostPosition)
        probability = busters.getObservationProbability(noisyDistance, t_dis)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            probability = self.getObservationProb(observation, pac_pos, pos, jail_pos) * self.beliefs[pos]
            self.beliefs[pos] = probability

        "*** END YOUR CODE HERE ***"
        if self.beliefs.total() != 0.0:
            self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        number = self.numParticles / len(self.legalPositions)
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        dict = {}
        result = DiscreteDistribution(dict)

        for particle in self.particles:
            if result[particle] == None:
                result[particle] = 1
            else:
                result[particle] += 1
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.legalPositions:
            probability = self.getObservationProb(observation, pac_pos, pos, jail_pos)
            belief[pos] *= probability
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.legalPositions:
            ...
            for key in newPosDist.keys():
                    dict[key] += newPosDist[key] * belief[particle]
        
        for i in range(self.numParticles):
            new_particles.append(dict.sample())
